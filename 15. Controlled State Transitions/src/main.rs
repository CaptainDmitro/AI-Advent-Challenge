...
// ---------------------------------------------------------------------
// Task state machine (structure unchanged from lesson 13; this lesson adds
// the two named gates below). Lesson 12's working memory held a task as a
// bare `Option<String>` name with no notion of progress; lesson 13
// formalized it into the FSM below. Here a task is a formal finite state
// machine: a `stage` (one of four gates), a free-text `step` describing
// progress within that stage, and a free-text `expected_action` naming
// what the FSM is currently waiting on. `paused` is orthogonal to `stage`
// - pausing works identically no matter which stage the task is in, and
// never loses stage/step/expected_action. Lesson 15 adds `plan_approved`
// and `validated`: two explicit gates, each flipped only by its own
// dedicated endpoint, that `advance_task` now checks *in addition to* the
// structural table below - see `stage_gate_error` further down.
// ---------------------------------------------------------------------
...
struct TaskFsm {
    ...
    #[serde(default)]
    expected_action: String,
    /// Set only by `Agent::approve_plan`, only while `stage == Planning`.
    /// Gates the `planning -> execution` edge - see `stage_gate_error`.
    /// Never set by `advance_task` itself and never inferred by the model.
    #[serde(default)]
    plan_approved: bool,
    /// Set only by `Agent::record_validation`, only while `stage ==
    /// Validation`. Gates the `validation -> done` edge - see
    /// `stage_gate_error`. Reset to `false` whenever a rework loop sends
    /// the task back to `execution`, since that validation run no longer
    /// describes the code it's about to re-check.
    #[serde(default)]
    validated: bool,
    /// Free-text note attached to the last `record_validation` call (e.g.
    /// why a check failed). Purely descriptive - nothing branches on it.
    #[serde(default)]
...
fn format_working_block(task: &Option<TaskFsm>, facts: &BTreeMap<String, String>) -> Option<String> {
    ...
    s.push_str(&format!(
        "Plan approved: {}\nValidation passed: {}\n",
        if task.plan_approved { "yes" } else { "no" },
        if task.validated { "yes" } else { "not yet" }
    ));
    if !task.validation_notes.is_empty() {
        s.push_str(&format!("Last validation note: {}\n", task.validation_notes));
    }
...
// ---------------------------------------------------------------------
// Controlled state transitions (this lesson's addition). Lesson 13 already
// refused a structurally invalid edge (e.g. planning straight to done) via
// `TaskStage::can_advance_to`. This lesson adds two named GATES on top of
// that table for the two edges the assignment calls out explicitly:
// `planning -> execution` now also requires the plan to have been approved
// through its own dedicated endpoint, and `validation -> done` now also
// requires a validation outcome to have been recorded as passing. Neither
// gate is a stage of its own and neither can be satisfied by `advance_task`
// itself - only `Agent::approve_plan` / `Agent::record_validation` can flip
// them, following the same "never inferred, only API-written" rule the FSM's
// stage has always followed.
// ---------------------------------------------------------------------

/// Checks the two gated edges. Returns `None` for every other edge
/// (including every edge `TaskStage::can_advance_to` already rejects
/// structurally - this only adds a *second* condition on top of an edge
/// that's already on the table).
fn stage_gate_error(task: &TaskFsm, target: TaskStage) -> Option<String> {
    match (task.stage, target) {
        (TaskStage::Planning, TaskStage::Execution) if !task.plan_approved => Some(
            "Cannot start execution: the plan has not been approved yet. Call POST \
             /api/task/approve_plan first."
                .to_string(),
        ),
        (TaskStage::Validation, TaskStage::Done) if !task.validated => Some(
            "Cannot mark the task done: validation has not passed yet. Call POST \
             /api/task/validate with {\"passed\": true} first."
                .to_string(),
        ),
        _ => None,
    }
...
/// Which of the two named gates a chat message asked to skip past.
...
impl StageGate {
    fn id(self) -> &'static str {
        match self {
            StageGate::PlanApproval => "plan_not_approved",
            StageGate::Validation => "not_validated",
        }
    }
}

/// Deliberately narrow phrase lists, exactly like an invariant's
/// `forbidden_terms`: they only catch the two transitions the assignment
/// names by example, in plain English, and exist so the two canonical
/// "trying to skip a stage" cases can be blocked - and demoed - without a
/// live model call. Everything subtler still rides on `format_working_block`'s
/// GATES instruction for the model itself to catch.
...
/// The hard, deterministic half of stage-transition enforcement - the
/// chat-facing counterpart to `stage_gate_error`. Scans the raw user
/// message for phrasing that asks for next-stage work the active,
/// unpaused task hasn't actually earned yet, and returns which gate is
/// missing so `Agent::respond` can refuse before the model is ever called.
fn find_blocked_stage_request(task: &Option<TaskFsm>, user_message: &str) -> Option<StageGate> {
    let task = task.as_ref()?;
    if task.paused {
        return None;
    }
    ...
    if task.stage == TaskStage::Planning
        && !task.plan_approved
        && IMPLEMENTATION_PHRASES.iter().any(|p| lower.contains(p))
    {
        return Some(StageGate::PlanApproval);
    }
    if task.stage != TaskStage::Done
        && !task.validated
        && FINALIZE_PHRASES.iter().any(|p| lower.contains(p))
    {
        return Some(StageGate::Validation);
    }
    None
}

/// The refusal text for a turn `find_blocked_stage_request` blocked. Not
/// LLM output, for the same reason `format_invariant_refusal` isn't:
/// assembled in plain Rust so it's guaranteed to name the right gate and
/// the right unlocking call, regardless of what the model would have said.
fn format_stage_gate_refusal(gate: StageGate, task: &TaskFsm) -> String {
    match gate {
        StageGate::PlanApproval => format!(
            "I can't start implementation yet on \"{}\" - the plan hasn't been approved. The \
             task is still in \"planning\"; call `POST /api/task/approve_plan` first, then \
             advance it to \"execution\".",
            task.name
        ),
        StageGate::Validation => format!(
            "I can't treat \"{}\" as finished yet - validation hasn't passed. Record the \
             outcome with `POST /api/task/validate` (`{{\"passed\": true}}`) once it \
             actually has, then advance the task to \"done\".",
            task.name
        ),
    }
}
...
struct MemoryReport {
    ...
    /// Set only on a turn `find_blocked_stage_request` blocked before it
    /// ever reached the model - which gate ("plan_not_approved" or
    /// "not_validated") blocked it.
    blocked_by_stage_gate: Option<String>,
}
...
impl Agent {
    ...
    fn compose_report(&self, actual: Option<Usage>, sent_context_tokens: u32) -> MemoryReport {
        self.compose_report_with(actual, sent_context_tokens, None, None)
    }
    ...
    async fn respond(&self, user_message: &str, options: ChatOptions) -> RespondResult {
        ...
        if let Some(violated) = self.find_violated_invariant(user_message) {
            ...
            self.persist_short_term();
            let memory = self.compose_report_with(None, sent_context_tokens, Some(violated.id.clone()), None);
            return RespondResult { reply, memory };
        }

        // The second deterministic pre-check, also before the model is ever
        // called: does the raw message ask for work belonging to a stage
        // the active task hasn't earned yet (see `find_blocked_stage_request`).
        // Runs after the invariant check but on exactly the same principle -
        // a hard, testable refusal in plain Rust for the cases a fixed
        // phrase list can actually name.
        if let Some(gate) = find_blocked_stage_request(&working_task, user_message) {
            let reply = format_stage_gate_refusal(gate, working_task.as_ref().unwrap());
            {
                let mut st = self.short_term.lock().unwrap();
                st.messages.push(Message {
                    role: "assistant".to_string(),
            ...
            self.persist_short_term();
            let memory = self.compose_report_with(None, sent_context_tokens, None, Some(gate.id().to_string()));
            return RespondResult { reply, memory };
        }
    ...
    /// Moves the active task to `target`, validated two ways: structurally
    /// against `TaskStage::can_advance_to` (a stray request can't drive the
    /// FSM into a state it isn't allowed to reach directly, e.g. `planning`
    /// straight to `done`), and then against `stage_gate_error` for the two
    /// edges that also require an explicit gate to have been passed first.
    /// Refuses while paused - resume first, so a stage change is never
    /// silently applied to a task the user thinks is frozen.
    fn advance_task(&self, target: TaskStage, step: String, expected_action: String) -> Result<(), String> {
        ...
        if let Some(err) = stage_gate_error(task, target) {
            return Err(err);
        }
        // Leaving validation for a rework pass invalidates whatever the
        ...
        if leaving_validation_for_rework {
            ...
            task.validation_notes = String::new();
        }
    ...
    /// The explicit, dedicated approval gate for the plan - never bundled
    /// into `advance_task` and never inferred by the model, matching the
    /// FSM's own "state only ever changes through an explicit call" rule.
    /// Only meaningful while the task is still in `planning`: approving
    /// after the task has already moved on would just be misleading, so
    /// it's rejected rather than silently accepted as a no-op.
    fn approve_plan(&self) -> Result<(), String> {
        let mut w = self.working.lock().unwrap();
        let task = w.task.as_mut().ok_or_else(|| "No task is currently active.".to_string())?;
        if task.paused {
            return Err("Task is paused; resume it before approving the plan.".to_string());
        }
        if task.stage != TaskStage::Planning {
            return Err(format!(
                "Plan approval only applies during \"planning\" (task is currently \"{}\").",
                task.stage.label()
            ));
        }
        if task.plan_approved {
            return Err("The plan is already approved.".to_string());
        }
    ...
    /// The explicit, dedicated validation outcome - never bundled into
    /// `advance_task` and never inferred by the model. Only meaningful
    /// while the task is in `validation`: recording an outcome from any
    /// other stage would describe a check that couldn't actually have run
    /// yet, so it's rejected rather than silently accepted.
    fn record_validation(&self, passed: bool, notes: String) -> Result<(), String> {
        let mut w = self.working.lock().unwrap();
        let task = w.task.as_mut().ok_or_else(|| "No task is currently active.".to_string())?;
        if task.paused {
            return Err("Task is paused; resume it before recording a validation outcome.".to_string());
        }
        if task.stage != TaskStage::Validation {
            return Err(format!(
                "Validation can only be recorded during \"validation\" (task is currently \"{}\").",
                task.stage.label()
            ));
        }
...
async fn approve_plan_task(State(agent): State<Arc<Agent>>) -> Result<Json<MemoryReport>, (StatusCode, String)> {
    agent.approve_plan().map_err(|e| (StatusCode::BAD_REQUEST, e))?;
    Ok(Json(agent.snapshot()))
}

async fn validate_task(
    State(agent): State<Arc<Agent>>,
    Json(req): Json<ValidateTaskRequest>,
) -> Result<Json<MemoryReport>, (StatusCode, String)> {
    agent
        .record_validation(req.passed, req.notes)
        .map_err(|e| (StatusCode::BAD_REQUEST, e))?;
    Ok(Json(agent.snapshot()))
}
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/task/approve_plan", post(approve_plan_task))
        .route("/api/task/validate", post(validate_task))
...
mod tests {
    ...
    fn temp_path(label: &str) -> String {
        ...
        std::env::temp_dir()
            .join(format!("ai-advent-lesson-15-{label}-{}-{}.json", std::process::id(), n))
            .to_string_lossy()
            .into_owned()
    }
    ...
    // -------------------------------------------------------------
    // Controlled state transitions, end to end through `respond` -
    // "the assistant can't skip a stage" verified at the chat layer,
    // not just at the `advance_task` API layer.
    // -------------------------------------------------------------

    #[tokio::test]
    async fn respond_refuses_implementation_before_plan_approval_without_calling_the_model() {
        // Same proof as the invariant test above: test_agent_no_invariants()
        // points at a bogus endpoint, so a clean, on-topic refusal (rather
        // than a connection-error reply) proves this never reached call_chat.
        let agent = test_agent_no_invariants();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();

        let result = agent
            .respond("Great, let's start coding this now.", ChatOptions { temperature: None, max_tokens: None })
            .await;

        assert!(result.reply.contains("approve_plan"));
        assert_eq!(result.memory.blocked_by_stage_gate, Some("plan_not_approved".to_string()));
        assert_eq!(result.memory.blocked_by_invariant, None);
        assert!(result.memory.actual.is_none());
        ...
        assert_eq!(messages.len(), 2);
        assert_eq!(messages[1].role, "assistant");
        assert!(messages[1].content.contains("approve_plan"));
    }

    #[tokio::test]
    async fn respond_does_not_block_implementation_once_the_plan_is_approved() {
        let agent = test_agent_no_invariants();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();

        // Falls through to call_chat, which fails fast against the bogus
        // endpoint - an error reply, not a stage-gate refusal.
        let result = agent
            .respond("Great, let's start coding this now.", ChatOptions { temperature: None, max_tokens: None })
            .await;

        assert_eq!(result.memory.blocked_by_stage_gate, None);
    }

    #[tokio::test]
    async fn respond_refuses_finishing_before_validation_without_calling_the_model() {
        let agent = test_agent_no_invariants();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        ...
        let result = agent
            .respond("Looks good to me, let's mark this done.", ChatOptions { temperature: None, max_tokens: None })
            .await;

        assert!(result.reply.contains("/api/task/validate"));
        assert_eq!(result.memory.blocked_by_stage_gate, Some("not_validated".to_string()));
        assert!(result.memory.actual.is_none());
    }

    #[tokio::test]
    async fn respond_does_not_block_a_stage_skip_phrase_while_the_task_is_paused() {
        // Paused freezes the stage-gate check too - a paused task's next
        // message shouldn't be misread as a request to skip ahead.
        let agent = test_agent_no_invariants();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.pause_task().unwrap();

        let result = agent
            .respond("Let's start coding this now.", ChatOptions { temperature: None, max_tokens: None })
            .await;
    ...
    fn advance_task_allows_validation_back_to_execution_for_rework() {
        ...
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        agent.record_validation(false, "one test still failing".to_string()).unwrap();
        ...
        assert_eq!(task.step, "fix the failing test");
        // A rework pass invalidates the prior (failing) validation record.
        assert!(!task.validated);
        assert_eq!(task.validation_notes, "");
    }
    ...
    // -------------------------------------------------------------
    // Controlled state transitions: the two named gates ("no
    // implementation before an approved plan", "no final without
    // validation") on top of the structural transition table.
    // -------------------------------------------------------------

    #[test]
    fn advance_task_rejects_execution_without_plan_approval() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        let err = agent
            .advance_task(TaskStage::Execution, "".to_string(), "".to_string())
            .unwrap_err();
        assert!(err.contains("plan has not been approved"));
        assert_eq!(agent.working_task().unwrap().stage, TaskStage::Planning);
    }

    #[test]
    fn advance_task_allows_execution_after_plan_approval() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent
    ...
    #[test]
    fn advance_task_rejects_done_without_validation() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        let err = agent.advance_task(TaskStage::Done, "".to_string(), "".to_string()).unwrap_err();
        assert!(err.contains("validation has not passed"));
        assert_eq!(agent.working_task().unwrap().stage, TaskStage::Validation);
    }

    #[test]
    fn advance_task_allows_done_after_validation_passed() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        agent.record_validation(true, "all checks pass".to_string()).unwrap();
        agent.advance_task(TaskStage::Done, "".to_string(), "".to_string()).unwrap();
        assert_eq!(agent.working_task().unwrap().stage, TaskStage::Done);
    }

    #[test]
    fn advance_task_rejects_done_after_a_failed_validation() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        agent.record_validation(false, "two tests failing".to_string()).unwrap();
        assert!(agent.advance_task(TaskStage::Done, "".to_string(), "".to_string()).is_err());
    }

    #[test]
    fn approve_plan_rejects_without_an_active_task() {
        let agent = test_agent();
        assert!(agent.approve_plan().is_err());
    }

    #[test]
    fn approve_plan_rejects_outside_the_planning_stage() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        let err = agent.approve_plan().unwrap_err();
        assert!(err.contains("planning"));
    }

    #[test]
    fn approve_plan_rejects_while_paused() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.pause_task().unwrap();
        assert!(agent.approve_plan().is_err());
    ...
    #[test]
    fn approve_plan_rejects_a_second_approval() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        assert!(agent.approve_plan().is_err());
    ...
    #[test]
    fn record_validation_rejects_outside_the_validation_stage() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        let err = agent.record_validation(true, "".to_string()).unwrap_err();
        assert!(err.contains("validation"));
    }

    #[test]
    fn record_validation_rejects_while_paused() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        agent.pause_task().unwrap();
        assert!(agent.record_validation(true, "".to_string()).is_err());
    }

    #[test]
    fn record_validation_stores_notes_and_updates_the_flag() {
        let agent = test_agent();
        agent.start_task("Ship it".to_string(), "".to_string(), "".to_string()).unwrap();
        agent.approve_plan().unwrap();
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        ...
        let task = agent.working_task().unwrap();
        assert!(!task.validated);
        assert_eq!(task.validation_notes, "one test still failing");
    }

    #[test]
    fn find_blocked_stage_request_detects_premature_implementation() {
        let task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        assert_eq!(
            find_blocked_stage_request(&Some(task), "let's start coding this now"),
            Some(StageGate::PlanApproval)
        );
    }

    #[test]
    fn find_blocked_stage_request_is_none_once_the_plan_is_approved() {
        let mut task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        task.plan_approved = true;
        assert_eq!(find_blocked_stage_request(&Some(task), "let's start coding this now"), None);
    }

    #[test]
    fn find_blocked_stage_request_detects_premature_finish() {
        let mut task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        task.stage = TaskStage::Validation;
        assert_eq!(
            find_blocked_stage_request(&Some(task), "great, let's mark this done"),
            Some(StageGate::Validation)
        );
    }

    #[test]
    fn find_blocked_stage_request_is_none_once_validated() {
        let mut task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        task.stage = TaskStage::Validation;
        task.validated = true;
        assert_eq!(find_blocked_stage_request(&Some(task), "great, let's mark this done"), None);
    }

    #[test]
    fn find_blocked_stage_request_is_none_while_paused() {
        let mut task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        task.paused = true;
        assert_eq!(find_blocked_stage_request(&Some(task), "let's start coding this now"), None);
    }
    ...
    #[test]
    fn find_blocked_stage_request_is_none_without_an_active_task() {
        assert_eq!(find_blocked_stage_request(&None, "let's start coding this now"), None);
    }

    #[test]
    fn find_blocked_stage_request_ignores_unrelated_messages() {
        let task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        assert_eq!(find_blocked_stage_request(&Some(task), "what's the weather like today?"), None);
    }

    #[test]
    fn format_stage_gate_refusal_names_the_task_and_the_unlocking_call() {
        let task = TaskFsm::new("Ship it".to_string(), "".to_string(), "".to_string());
        let refusal = format_stage_gate_refusal(StageGate::PlanApproval, &task);
        assert!(refusal.contains("Ship it"));
        assert!(refusal.contains("/api/task/approve_plan"));

        let refusal = format_stage_gate_refusal(StageGate::Validation, &task);
        assert!(refusal.contains("/api/task/validate"));
    }
    ...
    fn pause_works_from_every_stage() {
        ...
        agent.advance_task(TaskStage::Execution, "".to_string(), "".to_string()).unwrap();
        agent.advance_task(TaskStage::Validation, "".to_string(), "".to_string()).unwrap();
        agent.record_validation(true, "".to_string()).unwrap();
        agent.advance_task(TaskStage::Done, "".to_string(), "".to_string()).unwrap();
        agent.pause_task().unwrap();
        assert_eq!(agent.working_task().unwrap().stage, TaskStage::Done);
        assert!(agent.working_task().unwrap().paused);
