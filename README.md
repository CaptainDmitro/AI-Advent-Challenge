# AI Advent Challenge

Practice projects from an AI-focused course, one folder per lesson.

## Table of contents

| # | Lesson | Description |
|---|---|---|
| 01 | [Rust LLM Chat CLI](./01.%20Rust%20LLM%20Chat%20CLI) | Continuous CLI chat loop against an OpenAI-compatible LLM API, written in Rust |
| 02 | [Rust LLM Response Control CLI](./02.%20Rust%20LLM%20Response%20Control%20CLI) | Live commands to control response format, length, and stop conditions, to compare restricted vs. unrestricted answers |
| 03 | [Different Reasoning Approaches](./03.%20Different%20Reasoning%20Approaches) | Solving one task four ways (direct, step-by-step, self-authored prompt, expert panel) and comparing the results |
| 04 | [Model Version Comparison](./04.%20Model%20Version%20Comparison) | Same prompt sent to a weak, medium, and strong model in parallel, comparing response time, tokens, and cost |
| 05 | [Temperature Comparison](./05.%20Temperature%20Comparison) | Same prompt sampled 3x at temperature 0 / 0.7 / 1.2 to compare accuracy, creativity, and diversity |
| 06 | [First Agent](./06.%20First%20Agent) | A minimal chat agent that encapsulates the LLM request/response cycle behind an `Agent` entity |
| 07 | [Context Persistence](./07.%20Context%20Persistence) | The same agent, now saving and restoring its conversation history to a JSON file so it survives a restart |
| 08 | [Token Counting](./08.%20Token%20Counting) | The same agent, now reporting estimated and actual token usage per turn, a running history total, and cost |
| 09 | [Context Compression](./09.%20Context%20Compression) | The same agent, now folding everything older than the last N messages into a running summary, and reporting how many tokens that saves per turn vs. sending full history |
| 10 | [Context Management Strategies](./10.%20Context%20Management%20Strategies) | Three switchable, summary-free context strategies — sliding window, sticky key-value facts, and branching checkpoints — compared on the same spec-gathering scenario for quality, stability, and token cost |
| 11 | [Agent Memory Model](./11.%20Agent%20Memory%20Model) | Three independently-stored memory layers — short-term dialogue, task-scoped working memory, and a durable long-term profile — each with its own file and its own narrowly-scoped LLM extraction prompt, so what gets remembered where is an explicit, testable choice |
| 12 | [Personalization](./12.%20Personalization) | An explicit, user-authored personalization profile (style, tone, format, language, constraints) layered on top of lesson 11's memory model — written only by direct API calls, never inferred, and injected into every request so responses are verifiably different across profiles |
| 13 | [Task State Machine](./13.%20Task%20State%20Machine) | A task's state formalized as an explicit finite state machine — stage (planning → execution → validation → done), step, and expected action — with validated transitions, pause/resume orthogonal to stage, and the full state injected into every request so the agent never needs the task re-explained after a pause |
| 14 | [Invariant Guardrails](./14.%20Invariant%20Guardrails) | A set of binding invariants (architecture, tech decisions, stack constraints, business rules) stored apart from the dialogue and never LLM-written, injected into every request, and enforced two ways — a deterministic keyword pre-check that blocks a turn before it ever reaches the model, and a system-level instruction that makes the model itself check every request against the list and refuse, naming the invariant, when one conflicts |
| 15 | [Controlled State Transitions](./15.%20Controlled%20State%20Transitions) | Two named gates added on top of lesson 13's task FSM — no execution without an approved plan, no "done" without a passed validation — each flipped only by its own dedicated endpoint, enforced both at the API (`advance_task` refuses the jump) and in chat (a deterministic phrase check plus a model-facing instruction refuse and name the missing gate before the model, or the assistant, can skip ahead) |
| 16 | [MCP Connection](./16.%20MCP%20Connection) | A minimal MCP client on the official Rust SDK (`rmcp`) — connects to a Streamable HTTP MCP server (DeepWiki by default), performs the `initialize` handshake, and lists the server's tools with their input schemas; `cargo test` verifies connect + `tools/list` against an in-process MCP server |
| 17 | [First MCP Tool](./17.%20First%20MCP%20Tool) | An own MCP server (`rmcp` macros) exposing one described tool, `list_recent_commits`, over the public GitHub REST API at `/mcp`, plus a minimal agent that discovers it over Streamable HTTP, lets the LLM call it via function calling, and answers from the tool's result; `cargo test` covers registration, schema, `tools/call`, and the full agent → MCP → LLM loop end to end |
| 18 | [Scheduler and Background Tasks](./18.%20Scheduler%20and%20Background%20Tasks) | MCP tools with deferred and periodic execution — `watch_repo` (periodic collection of a repo's commits), `remind_me` (one-shot reminder), `list_jobs`/`cancel_job`, and `get_summary` (aggregated result over a time window) — persisted to a JSON file and run by a background scheduler loop, plus a built-in digest job that makes the agent report 24/7: it calls `get_summary` over MCP and the LLM writes the digest |
| 19 | [MCP Tool Composition](./19.%20MCP%20Tool%20Composition) | Three MCP tools built to be chained — `search` (GitHub repositories) → `summarize` (stats + LLM overview into a Markdown report) → `save_to_file` — passing data by artifact id with checksums, run automatically either as a fixed pipeline that wires each step from the previous step's `structuredContent` or by an agent that chains them itself, with every handoff re-verified afterwards (same checksum, grounded summary, identical bytes on disk) |
| 20 | [MCP Orchestration](./20.%20MCP%20Orchestration) | Three separate MCP servers — `crates` (crates.io), `github` (GitHub API) and `notes` (file storage) — each on its own Streamable HTTP endpoint with its own `serverInfo` and tools, registered in one list (`MCP_SERVERS`) and driven by an orchestrator agent that namespaces every tool as `<server>__<tool>`, routes each call to the server that owns it, survives a server being down, and runs a long cross-server flow (search → lookups → repo checks → write note → read back), with every run checked for server coverage, correct routing, call order (each argument traced to an earlier result) and note integrity |

## CI/CD

Every push to `master` runs [`.github/workflows/ci-cd.yml`](./.github/workflows/ci-cd.yml): it builds, tests, and lints every lesson automatically, then deploys whichever lesson is newest (web-app lessons only) to a VDS. See [`DEPLOYMENT.md`](./DEPLOYMENT.md) for the full architecture, the secrets/variables inventory, and how to redeploy an older lesson on demand.

## Working with AI coding agents

See [`AGENTS.md`](./AGENTS.md) for the conventions, environment constraints, and known-deferred issues an agent (or a new contributor) should know before touching this repo.
