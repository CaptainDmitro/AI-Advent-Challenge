...
// ---------------------------------------------------------------------
// Task state machine: this lesson's addition. Lesson 12's working memory
// held a task as a bare `Option<String>` name with no notion of progress.
...
/// unmodified. Order is deliberate: the profile (how to answer, set once
/// and rarely changed) comes first, then long-term facts (what's known
/// about the user, changes occasionally), then the task FSM (changes as
/// the task progresses), then the live conversation (changes every turn) -
/// each block earlier in the list is more stable than the one after it.
fn build_context(
    system_prompt: &str,
    profile: &UserProfile,
...
mod tests {
    ...
    fn temp_path(label: &str) -> String {
        ...
        std::env::temp_dir()
            .join(format!("ai-advent-lesson-13-{label}-{}-{}.json", std::process::id(), n))
            .to_string_lossy()
            .into_owned()
    }
    ...
    #[test]
    fn build_context_includes_profile_long_term_and_task_blocks_in_order() {
