...
/// facts" - short-term isn't a key-value store, it's the raw dialogue.
...
/// memory layer that has something to say, then the raw short-term
/// dialogue unmodified. The order mirrors how "durable" each layer is -
/// long-term (never changes within a session) before working memory
/// (changes once per task) before the live conversation (changes every
/// turn).
fn build_context(
    system_prompt: &str,
    ...
    let mut context = Vec::with_capacity(short_term_messages.len() + 3);
...
/// detail.
const LONG_TERM_MEMORY_PROMPT: &str = "You maintain a small, durable long-term memory about a user, meant to outlive any single task or conversation: who they are, standing preferences, recurring constraints, and decisions they explicitly asked to be remembered permanently. You will be given the CURRENT LONG-TERM FACTS as a JSON object and the LATEST TURN. Only extract something if it would still be true and relevant in a completely different, unrelated task later - never include a fact that only matters to the task at hand. Return the complete, updated facts as a single JSON object of string keys to string values, and nothing else - no prose, no markdown code fences, no commentary. Keep keys short and snake_case, and reuse an existing key when a new message updates the same fact. If nothing new or durable, return the CURRENT LONG-TERM FACTS unchanged.";
...
// ---------------------------------------------------------------------
// Persistence - each layer lives in its own file, loaded/saved
// independently, so the separation between them is visible on disk and a
// corrupt file for one layer can never take another down with it.
// ---------------------------------------------------------------------
...
/// The agent: owns all three memory layers, each behind its own mutex and
/// persisted to its own file. No method ever locks more than one layer's
/// mutex across an `.await` point, and every mutation is followed by a
/// persist call scoped to exactly the layer that changed.
struct Agent {
...
impl Agent {
    ...
    /// Builds a `MemoryReport` from whatever the three layers hold right
    /// now, given the token count of whatever request this report is
    /// describing (a real one just sent, or a hypothetical one for
    /// inspection).
    ...
    /// inspection, reset, task start/finish): what a request would look
    /// like *right now*, with no real call made.
    fn snapshot(&self) -> MemoryReport {
        let long_term = self.long_term_facts();
        let working_task = self.working_task();
        let working_facts = self.working_facts();
        let short_term = self.short_term_messages();
        let sent_messages = build_context(SYSTEM_PROMPT, &long_term, &working_task, &working_facts, &short_term);
    ...
    async fn respond(&self, user_message: &str, options: ChatOptions) -> RespondResult {
        ...
        match self.call_chat(sent_messages, options.temperature, options.max_tokens).await {
            Ok((reply, usage)) => {
                ...
                // Long-term extraction runs on every turn, task or no task.
    ...
    /// entirely - the clearest demonstration that "what goes where" is a
    /// deliberate choice, not something only an extraction prompt decides.
    ...
    /// Clears the current conversation only. Working memory (the active
    /// task) and long-term memory (what the agent durably knows about the
    /// user) are untouched - resetting a conversation does not make the
    /// agent forget who you are or what task it's in the middle of.
...
async fn main() {
    ...
    let task_note = agent
        ...
        .unwrap_or_default();
    println!(
        "Restored {} short-term message(s), {} working fact(s){task_note}, {} long-term fact(s).",
...
mod tests {
    ...
    fn temp_path(label: &str) -> String {
        ...
        std::env::temp_dir()
            .join(format!("ai-advent-lesson-11-{label}-{}-{}.json", std::process::id(), n))
            .to_string_lossy()
            .into_owned()
    }
    ...
    #[test]
    fn build_context_with_no_memory_is_just_system_and_dialogue() {
        let dialogue = vec![Message {
            role: "user".to_string(),
            content: "hi".to_string(),
        }];
        let ctx = build_context(SYSTEM_PROMPT, &BTreeMap::new(), &None, &BTreeMap::new(), &dialogue);
        assert_eq!(ctx.len(), 2);
    ...
    #[test]
    fn build_context_includes_long_term_and_working_blocks_in_order() {
        ...
        assert_eq!(ctx.len(), 4);
        assert_eq!(ctx[0].role, "system");
        assert!(ctx[1].content.contains("vegetarian"));
        assert!(ctx[2].content.contains("Plan a party"));
        assert_eq!(ctx[3].content, "hi");
    }
    ...
    fn save_json_then_load_or_default_round_trips() {
        ...
        let mut facts = BTreeMap::new();
        facts.insert("diet".to_string(), "vegetarian".to_string());
        let original = LongTermMemory { facts };
        save_json(&path, &original);
        let loaded: LongTermMemory = load_or_default(&path);
        assert_eq!(loaded.facts.get("diet"), Some(&"vegetarian".to_string()));
        let _ = std::fs::remove_file(&path);
    }

    #[test]
    fn new_starts_with_empty_layers_when_no_files_exist() {
    ...
    #[test]
    fn reset_clears_short_term_but_preserves_working_and_long_term() {
        let agent = test_agent();
        agent.short_term.lock().unwrap().messages.push(Message {
            role: "user".to_string(),
            content: "hi".to_string(),
